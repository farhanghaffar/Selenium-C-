...
{
    [TestClass]
    public class Reg_RequestPreAuthorizationTest : ROITestBase
    {

        public Reg_RequestPreAuthorizationTest() : base(ROITestArea.ROIAdmin)
        {
        ...
        public void ROI_Admin_CreateNew_Request()
        {
            try
            {
                ...
                Driver.logger.Log(Status.Pass, "Sucessfully Logged To Admin And Search Request ID.");
                RequestStatus.roiAssignReq();
                Driver.logger.Log(Status.Pass, "Sucessfully Assigned The Requester.");
                RequestStatus.roiAdminSearch();
                RequestStatus.roiSaveDonebtn();
                RequestStatus.applyRate();
                ROIAdminUpdateReqBlngDetls.pageFees();
                RequestStatus.roiCreateInvoice();
                Driver.logger.Log(Status.Pass, "Invoice Created Successfully.");
                RequestStatus.roiadmlogout();
                Driver.logger.Log(Status.Pass, "Successfully LogOut From Admin.");
